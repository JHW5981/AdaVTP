...
if __name__ == "__main__":
    ...
    parser.add_argument('--from_weight', default='v1-double-data-epoch-4', type=str, help="权重名称前缀（pretrain, full_sft, rlhf, reason, ppo_actor, grpo, spo）")
    ...
    parser.add_argument("--answer_file", type=str, default="/mnt/inaisfs/home/test3/jihuawei/workspace/AVA-llava_v1.5-jvis-arch-7/evaluation/playground/data/eval/MME/answers/llava-v1.5-7b.jsonl", help="答案路径")
    ...
    # # 把results中的key排一排序，结尾是Throughput的在一起，结尾是Accuracy的放一起，结尾是Relaxed_Accuracy的放一起，分别计算均值
    # from collections import OrderedDict

    # # 分类
    # throughput_keys = [k for k in results.keys() if k.endswith("Throughput")]
    # accuracy_keys = [k for k in results.keys() if k.endswith("Accuracy") and not k.endswith("Relaxed_Accuracy")]
    # relaxed_accuracy_keys = [k for k in results.keys() if k.endswith("Relaxed_Accuracy")]
    ...
    # throughput_avg = mean(throughput_keys)
    # accuracy_avg = mean(accuracy_keys)
    # relaxed_accuracy_avg = mean(relaxed_accuracy_keys)

    # # 构建新的有序results
    # sorted_results = OrderedDict()

    # # 原先的结果，剔除被归类的key，防止重复
    # left_keys = [k for k in results if k not in throughput_keys + accuracy_keys + relaxed_accuracy_keys]
    # for k in left_keys:
    #     sorted_results[k] = results[k]

    # for k in sorted(throughput_keys):
    #     sorted_results[k] = results[k]
    # if throughput_avg is not None:
    #     sorted_results["Throughput_Avg"] = throughput_avg

    # for k in sorted(accuracy_keys):
    #     sorted_results[k] = results[k]
    # if accuracy_avg is not None:
    #     sorted_results["Accuracy_Avg"] = accuracy_avg

    # for k in sorted(relaxed_accuracy_keys):
    #     sorted_results[k] = results[k]
    # if relaxed_accuracy_avg is not None:
    #     sorted_results["Relaxed_Accuracy_Avg"] = relaxed_accuracy_avg

    # results = sorted_results
    # Logger(results)
    # with open(os.path.join(args.eval_dir, f'main_finetune_results_{args.from_weight}.json'), 'w') as f:
