...
from .constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, VIDEO_TOKEN_INDEX, VISION_START_TOKEN_INDEX
...
class MyDataset(Dataset):
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        if not self.is_train:
            idx = sources[0]['question_id']
            image_files = [os.path.join(self.data_args.image_folder, sources[0]['image'])]
            prompt = sources[0]["text"]
            sources = [
                {
                    'id': idx,
                    'images': image_files,
            ...
            data_dict['visual_mask'] = (data_dict['input_ids'] == IMAGE_TOKEN_INDEX).float()
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, position_ids, visual_mask = tuple(
            [instance[key] for instance in instances]
            for key in ("input_ids", "labels", "position_ids", "visual_mask")
        )
        input_ids = [ids.squeeze(0) for ids in input_ids]
        visual_mask = [mask.squeeze(0) for mask in visual_mask]
        labels = [ids.squeeze(0) for ids in labels]
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
        visual_mask = torch.nn.utils.rnn.pad_sequence(
            visual_mask, batch_first=True, padding_value=1
        )
...
if __name__ == "__main__":
    ...
    with open("/mnt/inaisfs/home/test3/jihuawei/workspace/AVA/created_datasets/eval_downsample_ratio_1_dataset.json", "r") as f:
        data = json.load(f)['A-OKVQA']
